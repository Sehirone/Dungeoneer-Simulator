...
{
    srand(time(NULL));
    int sleepValue = argc == 2 ? atoi(argv[1]) : 200000;
...
// ***
    while(player1.isAlive())
    {
        ...
        if(firstSetup or encounter == leave)
        {
            ...
            journal1.addEntry("Entered " + to_string(currentDepth) + " dungeon floor!\n");
        }
        ...
        if(encounter == item)
        {
            ...
            switch(found)
            {
                case 1:
                    ...
                    info += "## It gave him " + to_string(player1.level()*8) + " XP points!\n";
                    journal1.addEntry("Found XP potion!\n");
                    break;
                case 2:
                    player1.heal(player1.maxHp()/5);
                    info += "## " + player1.name() + " found HP potion!\n";
                    info += "## It heald him for " + to_string(player1.maxHp()/5) + "!\n";
